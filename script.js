...
const State = {
  ...
  products: [],
  currentProduct: null,
  activeFilter: { category: null, subcategory: null }, // null = показати всі
};
...
const CATEGORY_MIGRATION_MAP = {
  'boxing':'martial','mma':'martial',
  'Единоборства':'martial','Единоборства / Бокс':'martial',
  'Бокс':'martial','MMA':'martial',
  'compression':'clothing','Компрессионная одежда':'clothing',
  'Спортивная одежда':'clothing',
  'Футбол':'football','Спорт товары':'sport',
  'Баскетбол':'basketball','Волейбол':'volleyball',
  'Плавание':'swimming',
  'Фитнес':'fitness','Фитнес йога':'fitness','Фитнес / Йога':'fitness',
  'Каратэ':'karate','Тхэквондо':'taekwondo',
  'Зимний спорт':'winter',
  'Товары для туризма и отдыха':'tourism','Туризм':'tourism',
};
function migrateProductCategories(products) {
  products.forEach(p => {
    const m = CATEGORY_MIGRATION_MAP[p.category];
    if (m) p.category = m;
  });
}
...
async function boot() {
  ...
  try {
    ...
    migrateProductCategories(products); // normalize old category values
    ...
    window.__fbOnProducts = function(products) {
      ...
      if (typeof migrateProductCategories === 'function') migrateProductCategories(products);
      State.products = products;
      renderProducts();
      renderCategories();
...
function openCategoryModal(cat) {
  ...
  /* Кнопка "Всі товари категорії" вгорі */
  const allBtn = document.createElement('div');
  allBtn.className = 'subcat-item';
  allBtn.style.cssText = 'border-bottom:2px solid #222;margin-bottom:4px;';
  allBtn.innerHTML = `
    <div class="subcat-left">
      <span class="subcat-icon">🔍</span>
      <span class="subcat-name" style="font-weight:600">${State.dict.show_all_cat || 'Все товары категории'}</span>
    </div>
    <div class="subcat-right">
      <span class="subcat-count">${cat.totalCount}</span>
      <span class="subcat-arrow">→</span>
    </div>`;
  allBtn.addEventListener('click', () => {
    State.activeFilter = { category: cat.id, subcategory: null };
    closeCategoryModal();
    setTimeout(() => {
      renderProducts();
  ...
  cat.subcategories.forEach(sub => {
    ...
    // Click on sub → фільтруємо товари по підкатегорії
    item.addEventListener('click', () => {
      State.activeFilter = { category: cat.id, subcategory: sub.name };
      closeCategoryModal();
      setTimeout(() => {
        renderProducts();
        document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' });
...
function renderProducts() {
  ...
  if (f.subcategory) {
    filtered = State.products.filter(p => p.subcategory === f.subcategory);
  } else if (f.category) {
    filtered = State.products.filter(p => p.category === f.category);
  }

  /* ── Оновлюємо заголовок секції ── */
  const sectionTitle  = $('#productsSectionTitle');
  const filterBar     = $('#productsFilterBar');
  const filterLabel   = $('#productsFilterLabel');
  const filterCount   = $('#productsFilterCount');

  if (filterBar) {
    if (f.subcategory || f.category) {
      filterBar.style.display = 'flex';
      if (filterLabel) {
        filterLabel.textContent = f.subcategory
          ? `${f.subcategory}`
          : CATEGORIES.find(c => c.id === f.category)?.name || f.category;
      }
      if (filterCount) filterCount.textContent = `${filtered.length} ${dict.stat_products || 'товаров'}`;
    } else {
  ...
  if (!filtered.length) {
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:#555;font-size:14px">
      ${dict.no_products || 'Товаров в этой подкатегории пока нет'}<br>
      <button onclick="window.clearProductFilter()" style="margin-top:16px;background:var(--red);color:#fff;border:none;padding:10px 24px;font-family:inherit;font-size:13px;cursor:pointer;letter-spacing:.1em;text-transform:uppercase">
        ${dict.show_all || 'Показать все товары'}
      </button>
    </div>`;
    return;
  }
  ...
  filtered.forEach((p, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="product-info">
        ...
        ${p.subcategory ? `<div style="font-size:11px;color:#555;margin-top:-8px;margin-bottom:8px;letter-spacing:.5px">${p.subcategory}</div>` : ''}
...
window.clearProductFilter = function() {
  State.activeFilter = { category: null, subcategory: null };
  renderProducts();
};
...
(function() {
  ...
  async function loadAdmProducts() {
      ...
      try {
        ...
        if (fbProds && fbProds.length > 0) {
          ...
          window.__admRefresh = (products) => {
            ...
            admFillCatFilter(); // keep category filter in sync
            admFilterProducts(); // re-apply all filters
            if ($i('admProdBadge')) $i('admProdBadge').textContent = admProducts.length;
          };
          return;
        }
    ...
    renderAdmTable();
    if ($i('admProdBadge')) $i('admProdBadge').textContent = admProducts.length;
    // Fill category filter from CATEGORIES
    admFillCatFilter();
  }

  function admFillCatFilter() {
    const sel = $i('admCatFilter');
    ...
    if (cur) {
      sel.value = cur;
      // Restore subcategory filter if was selected
      window.admCatFilterChanged && window.admCatFilterChanged();
    }
  }
  ...
  window.admFilterProducts = function() {
    ...
    const subcat = $i('admSubcatFilter')?.value || '';
    admFiltered = admProducts.filter(p => {
      const searchStr = (p.name + ' ' + (p.brand||'') + ' ' + (p.sku||'') + ' ' + (p.subcategory||'')).toLowerCase();
      // Match by id OR by stored category value (legacy support)
      const _admCatMap = {
        'clothing':['clothing','compression','Спортивная одежда'],
        'martial':['boxing','mma','martial','Единоборства','Единоборства / Бокс','Бокс','MMA'],
        'football':['football','Футбол'],
        'sport':['sport','Спорт товары'],
        'basketball':['basketball','Баскетбол'],
        'volleyball':['volleyball','Волейбол'],
        'swimming':['swimming','Плавание'],
        'fitness':['fitness','Фитнес','Фитнес йога','Фитнес / Йога'],
        'karate':['karate','Каратэ'],
        'taekwondo':['taekwondo','Тхэквондо'],
        'winter':['winter','Зимний спорт'],
        'tourism':['tourism','Товары для туризма и отдыха'],
      };
      const _validForCat = cat ? (_admCatMap[cat] || [cat]) : [];
      const catMatch = !cat || p.category === cat || _validForCat.includes(p.category);
      const subcatMatch = !subcat || p.subcategory === subcat;
      return searchStr.includes(q) && catMatch && subcatMatch;
    });
    admPage = 1;
    renderAdmTable();
  };

  /* When category filter changes — refill subcategory select */
  window.admCatFilterChanged = function() {
    const catId  = $i('admCatFilter')?.value || '';
    const subSel = $i('admSubcatFilter');
    if (!subSel) return;
    ...
    if (!catId) {
      ...
      // Also collect unique subcategories from actual products
      const _admCatMap = {
        'clothing':['clothing','compression','Спортивная одежда'],
        'martial':['boxing','mma','martial','Единоборства','Единоборства / Бокс','Бокс','MMA'],
        'football':['football','Футбол'],'sport':['sport','Спорт товары'],
        'basketball':['basketball','Баскетбол'],'volleyball':['volleyball','Волейбол'],
        'swimming':['swimming','Плавание'],'fitness':['fitness','Фитнес','Фитнес йога','Фитнес / Йога'],
        'karate':['karate','Каратэ'],'taekwondo':['taekwondo','Тхэквондо'],
        'winter':['winter','Зимний спорт'],'tourism':['tourism','Товары для туризма и отдыха'],
      };
      const validCats = _admCatMap[catId] || [catId];
      const productSubcats = [...new Set(
        admProducts
          .filter(p => p.category === catId || validCats.includes(p.category))
          .map(p => p.subcategory)
          .filter(Boolean)
      )];

      // Merge CATEGORIES subcats + actual product subcats
      const catSubNames = subs.map(s => s.name);
      const allSubcats = [
        ...subs.map(s => ({ icon: s.icon||'', name: s.name })),
        ...productSubcats
          .filter(n => !catSubNames.includes(n))
          .map(n => ({ icon: '📦', name: n }))
      ];

      subSel.innerHTML = '<option value="">Все подкатегории</option>' +
        allSubcats.map(s => `<option value="${s.name}">${s.icon} ${s.name}</option>`).join('');
      subSel.style.display = allSubcats.length ? 'block' : 'none';
      subSel.value = '';
    }
    admFilterProducts();
  };
  ...
  function renderAdmTable() {
    ...
    tbody.innerHTML = items.map(p => `
      <tr>
        ...
        <td>
          <span class="adm-td-category">${p.category||'—'}</span>
          ${p.subcategory ? `<div class="adm-td-subcategory">↳ ${p.subcategory}</div>` : ''}
        </td>
  ...
  /* ── Заполняем select категорий из CATEGORIES ── */
  function admFillCategorySelect() {
    const sel = $i('adm-p-category');
    if (!sel) return;
    const cur = sel.value; // save current selection
  ...
  /* ── Обновляем подкатегории при смене категории ── */
  window.admRefreshSubcats = function(selectedSub) {
    admFillCategorySelect();
    const catId  = $i('adm-p-category')?.value;
    const subSel = $i('adm-p-subcategory');
    if (!subSel) return;

    const cat = CATEGORIES.find(c => c.id === catId);
    if (!cat || !cat.subcategories || !cat.subcategories.length) {
      subSel.innerHTML = '<option value="">— нет подкатегорий —</option>';
    ...
    subSel.innerHTML = '<option value="">— выберите подкатегорию —</option>' +
      cat.subcategories.map(sub =>
        `<option value="${sub.name}">${sub.icon} ${sub.name}${sub.count ? ' ('+sub.count+')' : ''}</option>`
      ).join('');
  ...
  window.openAdmProductModal = function(product) {
    ...
    if (product) {
      ...
      $i('adm-p-category').value = product.category  || CATEGORIES[0]?.id || '';
      ...
      /* Обновляем подкатегории и выбираем сохранённую */
      window.admRefreshSubcats(product.subcategory || '');
    } else {
      ...
      $i('adm-p-category').value = CATEGORIES[0]?.id || '';
      $i('adm-p-badge').value    = '';
      if (delBtn) delBtn.style.display = 'none';
      if ($i('admImgPreviewWrap')) $i('admImgPreviewWrap').style.display = 'none';
      /* Обновляем подкатегории для первой категории */
      window.admRefreshSubcats('');
    }
  ...
  window.admSaveProduct = function() {
    ...
    const data = {
      ...
      category:    $i('adm-p-category')?.value || CATEGORIES[0]?.id || '',
      subcategory: $i('adm-p-subcategory')?.value || null,
...
(function() {
  ...
  window.openCategoryPage = function(cat) {
    currentCat = cat;
    currentSub = null;
    // Sync active filter so main products section also knows
    if (typeof State !== 'undefined') {
      State.activeFilter = { category: cat.id, subcategory: null };
    }
  ...
  function renderCatPage() {
    ...
    // Count
    const countLabel = currentSub
      ? `${products.length} ${dict.stat_products || 'товаров'} в «${currentSub}»`
      : `${products.length} ${dict.stat_products || 'товаров'}`;
    document.getElementById('catPageCount').textContent = countLabel;
  ...
  function getProductsForCat(cat, subFilter) {
    // Direct match: p.category === cat.id
    // Also support legacy catMap for backward compat
    const catMap = {
      // id → all possible stored category values
      'clothing':   ['clothing','compression','Спортивная одежда','Компрессионная одежда'],
      'martial':    ['boxing','mma','martial','Единоборства','Единоборства / Бокс','Бокс','MMA','боксёрские перчатки','Боксёрские перчатки'],
      'football':   ['football','Футбол'],
      'sport':      ['sport','Спорт товары','Спорт'],
      'basketball': ['basketball','Баскетбол'],
      'volleyball': ['volleyball','Волейбол'],
      'swimming':   ['swimming','Плавание'],
      'fitness':    ['fitness','Фитнес','Фитнес йога','Фитнес / Йога'],
      ...
      'taekwondo':  ['taekwondo','Тхэквондо'],
      'winter':     ['winter','Зимний спорт'],
      'tourism':    ['tourism','Товары для туризма и отдыха','Туризм'],
    };

    const validCats = catMap[cat.id] || [cat.id];

    let products = State.products.filter(p => {
      // Direct category match (new way) OR legacy catMap match
      return p.category === cat.id || validCats.includes(p.category);
    });

    if (subFilter) {
      // Exact subcategory match by stored p.subcategory field
      products = products.filter(p =>
        p.subcategory === subFilter ||
        p.subcategory === subFilter.trim()
      );
    }
  ...
  function renderCatFilters() {
    ...
    subs.forEach(sub => {
      ...
      const _validCats = _legacyMap[currentCat.id] || [currentCat.id];
      const subCount = State.products
        ? State.products.filter(p =>
            (p.category === currentCat.id || _validCats.includes(p.category)) &&
            p.subcategory === sub.name
          ).length
        : 0;
      html += `<button class="cat-filter-pill ${isActive ? 'active' : ''}"
        onclick="window.__setCatSub('${sub.name.replace(/'/g,"\\'")}')">
        ${sub.icon || ''} ${name}${subCount ? ` <span style="opacity:.6;font-size:11px">(${subCount})</span>` : ''}
      </button>`;
    });
  ...
  window.renderCategories = function() {
    ...
    CATEGORIES.forEach((cat, i) => {
      ...
      const catMap = {
        // After migration, all products should have cat.id values
        // But keep legacy values for safety
        'clothing':   ['clothing','compression','Спортивная одежда','Компрессионная одежда'],
        'martial':    ['boxing','mma','martial','Единоборства','Единоборства / Бокс','Бокс','MMA'],
        'football':   ['football','Футбол'],
        'sport':      ['sport','Спорт товары'],
      ...
      const prodCount = State.products
        ? State.products.filter(p =>
            ...
            validCats.some(v => v.toLowerCase() === (p.category||'').toLowerCase())
          ).length
        : 0;
...
function hookAdmCategoryChange() {
  ...
  if (!catSelect || catSelect.dataset.hooked) return;
  catSelect.dataset.hooked = '1';
  catSelect.addEventListener('change', () => {
    renderAdmSizes(catSelect.value, [], null);
    if (typeof window.admRefreshSubcats === 'function') {
      window.admRefreshSubcats('');
    }
  });
}
...
(function() {
  ...
  window.openAdminPanel = function() {
    ...
    setTimeout(() => {
      loadAndApplySubcats();
      initSubcatEditor();
      admFillCatFilter(); // fill category dropdown from CATEGORIES
    }, 150);
  };
